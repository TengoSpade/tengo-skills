---
name: pick-up
description: Resume a Gemini session from a previously written wrap-up file. Use when the user says "pick up where I left off", "resume session", "resume from wrap-up", "continue last session", "pick up", or invokes /pick-up. Finds the most recent wrap-up in ./wrap-ups/ (or accepts a specific path as argument), reads the "For Agent" section, verifies environment state still holds, re-populates todos, and stages the first action without executing it.
---
...
Use `read_file` to read the full file. Focus extraction on the `## For Agent` section — that's the operating context. From it, capture:
...
1. **Git branch.** If the file's header lists a branch, run `git branch --show-current` via `run_shell_command` and compare. On mismatch, tell the user — do not auto-switch branches.
...
Incorporate the result into the announcement:
...
3. **Key files exist.** For each `path/to/file.ext:line` reference in the agent section, confirm the file exists using `glob`. Warn on any missing file.
4. **Uncommitted files count.** If the file lists a count, run `git status --short | wc -l` and note drift in the announcement.

## Step 4 — Re-populate todos

If the agent section has an "Active todos" list, call `write_todos` to set the session's todo tracker to match. Preserve the item text verbatim. Mark all as pending unless the wrap-up indicates a different status.

Alternatively, use `tracker_create_task` if richer task management is preferred.

## Step 5 — Consult memory

Check Gemini's persistent memory for any facts that bear on the work described in the wrap-up (user role, project constraints, feedback rules). Surface those in the announcement so the user sees the full operating context.
...
If a chain predecessor was detected: include it in the announcement as shown. If the user says "load chain": use `read_file` to read the predecessor's `## For Agent` section and surface it as additional context before beginning work. If the predecessor file is not found, tell the user and continue with the current wrap-up only.
